...
var $author$project$Main$SavePredictions = {$: 'SavePredictions'};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'id',
				$elm$json$Json$Encode$int(prediction.id)),
				_Utils_Tuple2(
				'name',
	...
	function (predicate, update, list) {
		if (!list.b) {
			...
			return predicate(first) ? A2(
				...
				$elm$core$List$cons,
				first,
				A3($author$project$Utils$findAndUpdate, predicate, update, rest));
		}
	});
var $author$project$Main$setState = F3(
	function (idx, state, model) {
		return _Utils_update(
			model,
			{
				predictionList: A3(
		...
		var updatedPredictionList = A3(
			...
			function (pred) {
				return _Utils_update(
					pred,
					{
						difficulty: _Utils_Tuple2(pred.difficulty.a, reportedDifficulty)
					});
			},
			model.predictionList);
		return _Utils_update(
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'SetState':
				...
				var state = msg.b;
				return _Utils_Tuple2(
					A3($author$project$Main$setState, id, state, model),
					$author$project$Main$savePredictions(
			...
			if (_v0.$ === 'Just') {
				if (_v0.a.$ === 'NewPrediction') {
					...
					if (idx.$ === 'Just') {
						var num = idx.a;
						return A2(
							$author$project$Main$appView,
							$author$project$Main$reportView(num),
							model);
					} else {
						return _List_fromArray(
							[
				...
				return A2($author$project$Main$appView, $author$project$Main$predictionListView, model);
			}
		}(),
		title: 'BET'
	};
